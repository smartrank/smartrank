 ...
 */
public class JsonReaderFactory extends SearchCriteriaReaderFactory {

    @Override
    public boolean accepts(final String criteria) {
        return criteria.trim().startsWith("{") && criteria.trim().endsWith("}");
    }

    @Override
    protected SearchCriteriaReader newInstance(final String context, final String criteria) throws IOException {
        return new JsonReader(context, criteria);
    }
}
